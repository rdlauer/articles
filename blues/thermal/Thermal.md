...
*Note that the Python code for generating images is presented a little further down in this tutorial, but it's also available [in this GitHub repository]().*

With the complete set of images uploaded to my project in Edge Impulse studio, I had to **label** the individual images as to what they represented:
...
Using the default options provided, I trained my ML model based on the aforementioned dataset and configurations. The initial output of this process showed me that my model was going to be remarkably accurate (the testing is based on Edge Impulse pre-allocating images to a "training" dataset).
...
I then needed to [install some dependencies](https://docs.edgeimpulse.com/docs/raspberry-pi-4#2-installing-dependencies) for using the Edge Impulse runner on the Pi. Note that the instructions provided are for the Raspberry Pi 4, but they ran smoothly for me on the Pi Zero 2!
...
The last Edge Impulse step involved running this command to build and download the Edge Impulse model file (`modelfile.eim`) to the Pi:

```
edge-impulse-linux-runner --download modelfile.eim
```

I knew all would be right with the world when I saw the following in the terminal:
...
One trick with using the MLX90640 on the Raspberry Pi is we have to use the `adafruit_blinka` library. This translates the CircuitPython hardware API to whatever library the Pi provides. The advantage here is any code that runs on CircuitPython will run smoothly on a Raspberry Pi!

You can (and should) review the full setup instructions provided by Adafruit. However, they can also be boiled down to the following terminal commands:
...
The only other dependencies to be installed were the Python SDK for the cellular Notecard (and `python-periphery` for the Pi to communicate with the Notecard over I2C):
...
> **NOTE:** All of the code used for this project is available in this GitHub repository.

The first Python I "wrote" was actually, ahem, ["borrowed" from Adafruit](https://github.com/adafruit/Adafruit_CircuitPython_MLX90640/blob/main/examples/mlx90640_camtest.py). That being a script used to access the thermal camera, generate a color-coded image, and save it to the filesystem. Here is a highly-abridged version of the code:
...
**1)** Call the `takePicture` method in `thermal.py` and loads the image into a `cv2` image object:
...
One last reminder that the full Python code for this solution is available in this GitHub repository!

So in code above I mentioned I "sync [the JSON data] with the cloud". Let's see *where* exactly that data goes.

## Building a Cloud Dashboard

Since the Notecard is a device-to-cloud data pump, it doesn't live on the public Internet (making it an extremely secure device) and therefore needs a proxy with which to sync data. Enter Blues Wireless Notehub!
...
The JSON created (`note_body`), is saved on the Notecard as a Note. See the `file` parameter? That is a Notefile - one Notefile can store multiple Notes. As soon as the Notecard makes a connection to Notehub, any stored Notefiles get synced with the cloud along with some session-based metadata:

![thermal notehub events](notehub-events.png)

> **NOTE:** (Pun intended!) The [Notecard and Notehub APIs](https://dev.blues.io/reference/notecard-api/introduction/?utm_source=hackster&utm_medium=web&utm_campaign=featured-project&utm_content=thermal) are JSON-based. [Get a sneak peek of the API](https://dev.blues.io/?utm_source=hackster&utm_medium=web&utm_campaign=featured-project&utm_content=thermal#sneakpeek) and view all the developer resources provided at dev.blues.io.

With my data synced with Notehub, it was time to make some sense of it and build out a dashboard where I could actually view a visual representation of the data. In this case I used Ubidots which I've used numerous times in the past to create really engaging dashboards:

![example ubidots dashboard](ubidots-dashboard.png)

By following the comprehensive routing tutorial provided by Blues Wireless, I created a simple route from Notehub to Ubidots by passing an endpoint URL, authorization header, and a **JSONata expression** to transform my data on the fly:
...
...into the following optimized payload delivered to Ubidots:
...
> Take a look at the JSONata guide provided by Blues Wireless.
...
All that being said, if you're looking for some next steps, I can offer:

- Get your own Blues Wireless starter kit for cellular or Wi-Fi.
- Take a look at the Edge Impulse getting started resources.
